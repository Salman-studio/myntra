...
export default function Bag() {
  ...
  const fetchproduct = async () => {
    if (user) {
      try {
        setIsLoading(true);
        const response = await api.get(`/api/bag/${user._id}`);
        setbag(response.bag || response);
      } catch (error) {
        console.log(error);
        setIsLoading(false);
      } finally {
  ...
  const handleQuantityChange = async (itemid: any, newQuantity: number) => {
    ...
    try {
      await api.put(`/api/bag/${itemid}`, { quantity: newQuantity });
      fetchproduct();
    } catch (error) {
      console.log(error);
    }
  ...
  const handledelete=async(itemid:any)=>{
    try {
      await api.delete(`/api/bag/${itemid}`)
      fetchproduct();
    } catch (error) {
      console.log(error)
    }
